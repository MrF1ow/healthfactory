...
The first visit shows household setup. After that, the same route is sign-in. Household settings live at `/settings`. Each person edits their own profile at `/profile`. Home shows today's macro targets against logged meals for the signed-in person.
...
- `meal_logs`: append-only meal entries. Household members can read. Insert is self-only and `source` must be `human` from a signed-in person. Bots write via MCP with `source: bot`.
